...
C_in_df=pd.DataFrame(np.tile(C_in,(highest_stage-starting_stage+1,1)), columns=Conc_in_labels)
q_in_df=pd.DataFrame(np.tile(q_in,(highest_stage-starting_stage+1,1)), columns=q_in_labels) 

C_out_df=pd.DataFrame(C_out_list, columns=Conc_out_labels)
q_out_df=pd.DataFrame(q_out_list, columns=q_out_labels)
result = pd.concat([stages_df,C_in_df, q_in_df, C_out_df, q_out_df], axis=1)
...
print(result)
result.to_csv('countercurrent_constrained_purity_analysis_results_'+str(total_conc_ppm_mass)+'_ppm_'+str(PGM_labels)+'_PGMs_'+str(ligand)+'_ligand.csv', index=False)
